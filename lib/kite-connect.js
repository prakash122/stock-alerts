...
const getHistoricalDataForTradingSymbols = async (
    ...
    const generatePromises = function () {
        ...
        return _wrapEx(
            ...
            .catch((e) => {
                failedList.push(tradingSymbol);
                console.error('Failed', tradingSymbol, interval, e.message);
            });
    };
...
const getHistoricalData = async () => {
    ...
    try {
        ...
        notifyOnSlack('Day is completed, Failed ones');
        notifyOnSlack(JSON.stringify(failedList, null, 2));
        failedList = [];
        await getHistoricalDataForTradingSymbols(
            stockNameMap,
            INTERVALS.minute60
        );
        notifyOnSlack('Hour is completed, Failed ones');
        notifyOnSlack(JSON.stringify(failedList, null, 2));
        failedList = [];
        await getHistoricalDataForTradingSymbols(
            stockNameMap,
        ...
        notifyOnSlack('15 min is completed, Failed ones');
        notifyOnSlack(JSON.stringify(failedList, null, 2));
        failedList = [];
    } catch (error) {
        console.log('Errors for stockNameMap', stockNameMap, error);
    }
};
